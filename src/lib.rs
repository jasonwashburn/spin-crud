...
#[http_component]
fn handle_spin_crud(req: Request) -> Result<Response> {
    ...
    match api_from_request(req) {
        ...
        Api::Delete(id) => handle_delete(&cfg.mysql_address, id),
        _ => not_found(),
    }
}
...
fn handle_delete(adr: &str, id: u64) -> Result<Response> {
    let statement = "DELETE FROM Products WHERE Id = ?";
    let params = vec![ParameterValue::Uint64(id)];
    // TODO: Check if the product exists before deleting it
